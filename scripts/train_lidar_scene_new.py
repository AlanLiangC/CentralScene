...
from model.SGDiff_new import SGDiff
...
parser.add_argument('--network_type', default='echoscene', choices=['echoscene', 'echolayout', 'lidarscene'], type=str)
parser.add_argument('--diff_yaml', default='../config/full_mp_new.yaml', type=str,
...
def parse_data(data):
    ...
    lidar_range = torch.cat(data['range_image'], dim=0).cuda().unsqueeze(dim=1)
    lidar_mask = torch.cat(data['range_mask'], dim=0).cuda().unsqueeze(dim=1)
    ...
    return enc_objs, enc_triples, encoded_enc_f, encoded_enc_text_feat, encoded_enc_rel_feat,\
           ...
           encoded_dec_f, encoded_dec_text_feat, encoded_dec_rel_feat, dec_objs_to_scene, dec_triples_to_scene, missing_nodes, manipulated_nodes, lidar_range, lidar_mask
        ...
        for epoch in range(start_epoch, args.nepoch):
            ...
            for i, data in enumerate(tqdm(dataloader)):
                ...
                try:
                    ...
                    lidar_range, lidar_mask = parse_data(data)
                ...
                obj_selected, shape_loss, layout_loss, loss_dict = model.forward_mani(enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat,
                                               ...
                                               dec_objs_to_scene, missing_nodes, manipulated_nodes, lidar_range, lidar_mask)

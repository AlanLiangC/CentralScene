...
from model.networks.diffusion_lidm.layout2scene import LayoutToScene
from model.networks.diffusion_lidm.box2condition import Box2Condition
...
class LiScDiffModel(nn.Module):
    def __init__(self, vocab, diff_opt, diffusion_bs=8, embedding_dim=128, batch_size=32,
                 ...
                 use_angles=False,
                 use_clip=True):
        super(LiScDiffModel, self).__init__()
        ...
        ## shape branch
        box2condition = self.diff_cfg.shape_branch.pop('cond_convert_config')
        box2condition.in_channels = self.out_dim_manipulator * 2
        self.box2condition = Box2Condition(box2condition)
        self.ShapeDiff = LayoutToScene(**self.diff_cfg.shape_branch)
    ...
    def forward(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, dec_objs,
                dec_triples, dec_boxes, dec_text_feat, dec_rel_feat, dec_objs_to_scene, missing_nodes, manipulated_nodes, dec_angles, range_point, range_mask):
        ...
        # scene
        # condition project node to range view TODO: Other format for projecting
        shape_diff_dict = self.prepare_condition(dec_boxes, dec_angles, torch.cat([obj_embed_, latent_obj_vecs], dim=-1), range_point, range_mask)
        Shape_loss, Shape_loss_dict = self.ShapeDiff.training_step(shape_diff_dict)

        loss_dict = {**Shape_loss_dict, **Layout_loss_dict}

        return None, Shape_loss, Layout_loss, loss_dict

    def prepare_condition(self, dec_boxes, dec_angles, features, range_point, range_mask):
        shape_diff_dict = {}
        shape_diff_dict.update(
            {
                'image' : range_point,
                'mask' : range_mask,
            }
        )
        layout_condition = self.box2condition(dec_boxes, dec_angles, features)
        shape_diff_dict['layout'] = layout_condition

        return shape_diff_dict
